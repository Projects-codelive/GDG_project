...
export default function PostGrid({ directoryId }) {
    ...
    const getPlatformColor = (platform) => {
        switch (platform) {
            ...
            case 'youtube': return 'from-cherry to-apricot';
            default: return 'from-camel to-apricot';
        }
    };
        ...
        <div className="flex-1 p-4 overflow-y-auto bg-slate-50 dark:bg-slate-950">
            <div className="max-w-7xl mx-auto">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-0.5">Saved Posts</h2>
                        <p className="text-xs text-slate-600 dark:text-slate-400">{posts.length} {posts.length === 1 ? 'post' : 'posts'} saved</p>
                    </div>
                </div>
                ...
                {posts.length === 0 ? (
                    <div className="text-center py-12">
                        <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-apricot/20 to-cherry/20 dark:from-apricot/10 dark:to-cherry/10 mb-3">
                            <svg className="w-8 h-8 text-apricot dark:text-apricot" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z" />
                            </svg>
                        </div>
                        <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-1">No posts yet</h3>
                        <p className="text-sm text-slate-600 dark:text-slate-400 max-w-md mx-auto">Start saving posts from LinkedIn, Twitter, and Instagram using the Linko extension!</p>
                    </div>
                ) : (
                            ...
                            >
                                ...
                                <div className="p-5 flex-1 flex flex-col">
                                    ...
                                    <div className="flex items-center justify-between mb-3">
                                        <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-gradient-to-r ${getPlatformColor(post.platform)} text-white text-xs font-medium shadow-sm`}>
                                    ...
                                    {post.authorName && (
                                        <div className="flex items-center gap-2 mb-3">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <p className="font-semibold text-sm text-slate-900 dark:text-white truncate">{post.authorName}</p>
                                                {post.authorHandle && (
                                                    <p className="text-xs text-slate-500 dark:text-slate-400 truncate">@{post.authorHandle}</p>
                                                )}
                                            </div>
                                        </div>
                                    )}
